...
export const BlockCssFunc = ( props ) => {
	...
	const attrName = attrNameTemplate
		? getAttrName( attrNameTemplate, _attrName )
		: _attrName;
	const prefix = 'guten';
	...
	const getValue = ( attrName, device, state ) => {
		const unitAttrName = getAttributeName(
			`${ attrName }Unit`,
			device,
			state
		);
		...
		if ( value === '' && ( device === 'tablet' || device === 'mobile' ) ) {
			const desktopUnit =
				attributes[
					getAttributeName( `${ attrName }Unit`, 'desktop', state )
				];
			const tabletUnit =
				attributes[
					getAttributeName( `${ attrName }Unit`, 'tablet', state )
				];
			if ( device === 'tablet' ) {
				unit = desktopUnit;
			} else if ( device === 'mobile' ) {
				const tabletValue =
					attributes[ getAttributeName( attrName, 'tablet', state ) ];
				if ( tabletValue !== '' ) {
					unit = tabletUnit;
				} else {
					unit = desktopUnit;
		...
		if ( clampCallback && responsive ) {
			const desktopValue =
				attributes[ getAttributeName( attrName, 'desktop', state ) ];
			const tabletValue =
				attributes[ getAttributeName( attrName, 'tablet', state ) ];
			if ( value === '' || typeof value === 'undefined' ) {
				if ( device === 'tablet' ) {
					...
					value = clampCallback(
						tabletValue !== '' ? tabletValue : desktopValue,
						getAttribute,
						device,
						state,
						unit,
		...
		if ( unit && ! format.includes( '%s' ) ) {
			// Note: this will only work for non-objects.
	...
	const hover = hoverCallback
		? hoverCallback( getAttribute, attributes )
		: _hover;
	...
	const hasDesktop =
		...
		( Array.isArray( responsive ) &&
			responsive.find( ( s ) => s.startsWith( 'desktop' ) ) );
	const hasTablet =
		responsive === 'all' ||
		( Array.isArray( responsive ) &&
			responsive.find( ( s ) => s.startsWith( 'tablet' ) ) );
	const hasMobile =
		responsive === 'all' ||
		( Array.isArray( responsive ) &&
			responsive.find( ( s ) => s.startsWith( 'mobile' ) ) );

	const hasHover =
		hover === 'all' ||
		( Array.isArray( hover ) && hover.includes( 'hover' ) );
	const hasParentHover =
		hover === 'all' ||
		( Array.isArray( hover ) && hover.includes( 'parent-hover' ) );
	const hasCollapsed =
		hover === 'all' ||
		( Array.isArray( hover ) && hover.includes( 'collapsed' ) );
	...
	if ( hasDesktop ) {
		...
		if ( hasParentHover ) {
			valueDesktopParentHover = getValue(
				attrName,
				'desktop',
				'parent-hover'
			);
		...
		if ( hasCollapsed ) {
			valueDesktopCollapsed = getValue(
				attrName,
				'desktop',
				'collapsed'
			);
	...
	if ( hasTablet ) {
		...
		if ( hasParentHover ) {
			valueTabletParentHover = getValue(
				attrName,
				'tablet',
				'parent-hover'
			);
	...
	if ( hasMobile ) {
		...
		if ( hasParentHover ) {
			valueMobileParentHover = getValue(
				attrName,
				'mobile',
				'parent-hover'
			);
	...
	let selector = selectorCallback
		? selectorCallback( getAttribute, attributes, clientId, props )
		: _selector;
	let hoverSelector = hoverSelectorCallback
		? hoverSelectorCallback( getAttribute, attributes, clientId )
		: _hoverSelector;
	...
	if ( hasCollapsed ) {
		if ( generateForAllBlockStates ) {
			collapsedSelector = prependClass(
				selector,
				`%h :where(.${ prefix }-block-accordion.${ prefix }--is-open) .%s`
			);
		} else if ( blockState === 'collapsed' ) {
			collapsedSelector = prependClass(
				selector,
				`:where(.${ prefix }-block-accordion.${ prefix }--is-open) .%s`
			);
		} else {
			collapsedSelector = prependClass(
				selector,
	...
	if ( hasParentHover ) {
		if ( generateForAllBlockStates ) {
			parentHoverSelector = [
				prependClass( selector, `%h.%s.${ prefix }--is-hovered` ),
				prependClass(
					selector,
					`:where(.${ prefix }-hover-parent:hover, .${ prefix }-hover-parent.${ prefix }--is-hovered) .%s`
				),
			];
		} else if ( blockState === 'parent-hover' ) {
			parentHoverSelector = prependClass(
				selector,
				`.%s.${ prefix }--is-hovered`
			);
		} else {
	...
	if ( hasHover ) {
		...
		if ( selectorHasDataBlock ) {
			...
			if ( generateForAllBlockStates ) {
				hoverSelector = [
					prependClass( selector, `%h.%s.${ prefix }--is-hovered` ),
					hoverSelector || prependClass( selector, '.%s:hover' ),
				];
			} else if ( blockState === 'hover' ) {
				// eslint-disable-line no-lonely-if
				// In editor, always use the `selector` instead of the hoverSelector.
				hoverSelector = prependClass(
					selector,
					`.%s.${ prefix }--is-hovered`
				);
			} else {
	...
	//
	let blockUniqueClassName = getBlockUniqueClassname(
		attributes.uniqueId,
		clientId
	);
	if ( instanceId ) {
		...
		if ( typeof selector === 'string' ) {
			...
			parentHoverSelector =
				typeof parentHoverSelector === 'string'
					? parentHoverSelector.replace(
							/[^^?](.%s)([^-])/g,
							`$1-${ instanceId }$2`
					  )
					: parentHoverSelector.map( ( s ) =>
							s.replace(
								/[^^?](.%s)([^-])/g,
								`$1-${ instanceId }$2`
							)
					  );
			collapsedSelector = collapsedSelector.replace(
				/[^^?](.%s)([^-])/g,
				`$1-${ instanceId }$2`
			);
		}
	...
	if ( ! props.editorMode ) {
		blockUniqueClassName = applyFilters(
			'guten-css.block-css.uniqueClass.save',
			blockUniqueClassName,
			attributes
		);
	} else {
		blockUniqueClassName = applyFilters(
			'guten-css.block-css.uniqueClass.edit',
			blockUniqueClassName
		);
	}
	...
	if ( hasHover ) {
		hoverSelector = prependCSSClass(
			hoverSelector,
			blockUniqueClassName,
			blockUniqueClassName,
			wrapperClass,
			generateForAllBlockStates ? `.${ prefix }-preview-state--hover` : ''
		);
	}
	if ( hasParentHover ) {
		parentHoverSelector = prependCSSClass(
			parentHoverSelector,
			blockUniqueClassName,
			blockUniqueClassName,
			wrapperClass,
			generateForAllBlockStates
				? `.${ prefix }-preview-state--parent-hover`
				: ''
		);
	}
	if ( hasCollapsed ) {
		collapsedSelector = prependCSSClass(
			collapsedSelector,
			blockUniqueClassName,
			blockUniqueClassName,
			wrapperClass,
			generateForAllBlockStates
				? `.${ prefix }-preview-state--collapsed`
				: ''
		);
	}
	...
	if ( ! props.editorMode ) {
		css = applyFilters(
			'css-generator.block-styles.save',
			css,
			blockUniqueClassName,
			attributes
		);
...
function createCssEdit(
	...
	const cleanedRuleName = rule.replace(
		/^(--?)?(.*?$)/,
		( matches, dashes, rule ) => `${ dashes || '' }${ kebabCase( rule ) }`
	);
 ...
 */
function addCssToCssSaveObject(
	selector,
	rule,
	value,
	device = 'desktop',
