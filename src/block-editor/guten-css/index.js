...
export class GutenStyleGenerator {
	...
	addBlockStyle( _attrName, blockStyle ) {
		...
		const attrName = blockStyle.attrNameTemplate
			? getAttrName( blockStyle.attrNameTemplate, _attrName )
			: _attrName;
		...
		if ( blockStyle.valuePreCallback ) {
			if ( ! isFunction( blockStyle.valuePreCallback ) ) {
				console.error(
					`Guten CSS: valuePreCallback for attribute ${ _attrName } is not a function.`
				);
			}
	...
	getBlockStyles( attrNames, getNonCssAttributes = false ) {
		if ( ! attrNames ) {
			return this._orderedStyles.reduce(
				( blockStyles, blockStyle, index ) => {
		...
		const blockStyles = attrNames.reduce( ( blockStyles, attrName ) => {
			...
			if (
				! blockStyles[ rootAttrName ] &&
				this._blockStyles[ rootAttrName ]
			) {
		...
		if ( getNonCssAttributes && attrNames ) {
			const nonCssAttrs = attrNames.filter(
				( attrName ) =>
					! ( getRootAttrName( attrName ) in blockStyles ) &&
					attrName !== 'uniqueId'
			);
	...
	#generateBlockStylesForEditor( attributes, blockStyles, args ) {
		...
		return applyFilters(
			'guten-css.block-css.edit',
			output,
			getBlockUniqueClassname( attributes.uniqueId, args.clientId )
		);
	}
	...
	generate( attributes, options = {} ) {
		...
		if ( editorMode ) {
			return this.#generateBlockStylesForEditor(
				attributes,
				blockStyles,
				{
					blockState,
